...
const Form = () => {
  ...
  const errorHandler = (text) => {
    ...
    if (text.length > 20) {
      ...
      return false;
        ...
        setTextErrors((prev) => 
            prev.filter((item) => item !== "Too long")
        );
    if (text.length < 5) 
        ...
        setTextErrors((prev) => 
            prev.filter((item) => item !== "Too short"));
    if (!text.includes("@")) {
        if (!textErrors.includes("No @"))
        setTextErrors((prev) => [...prev, "No @"]);}
    else 
        setTextErrors((prev) => prev.filter((item) => item !== "No @"));
  ...
  const otherErrorHandler = (text) => {
    ...
    setOtherTextErrors((prev) => 
            prev.filter((item) => item !== "Too long Pass")
        );
    if (text.length < 8) 
    ...
    setOtherTextErrors((prev) => 
            prev.filter((item) => item !== "Too short Pass"));
      ...
      <Box display={"grid"} gridTemplateColumns="repeat(2, 1fr)">
        <form onSubmit={handleSubmit}>
          ...
          <Box display={"flex"} gap={3}>
            ...
            <Button variant="contained" type="reset" onClick={() => setValue("")}>Reset</Button>
          </Box>
        </form>
        ...
        <form onSubmit={otherHandleSubmit}>
          ...
          <Box display={"flex"} gap={3}>
            <Button variant="contained" type="submit">Submit</Button>
            <Button variant="contained" type="reset" onClick={() => setOtherValue("")}>Reset</Button>
          </Box>
        </form>
      </Box>
    </>
